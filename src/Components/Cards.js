      ...
      {loading ? <div className="w-full flex justify-center items-center h-96"> <ThreeDots height={40} color="white"/> </div> :
       data.map((e,i)=>{
        return (
            ...
            <Link to={`/detail/${e.id}`}><div key={i} className="bg-gray-900 shadow-lg
            text-sm font-medium p-2 hover:-translate-y-3 cursor-pointer  mt-6 transition-all duration-500 ">
